...
class Task:
    ...
    def update(task_id, update_data):
        # Normalize due_date to datetime for consistent storage.
        if "due_date" in update_data:
            due_date_val = update_data.get("due_date")
            if due_date_val:
                if isinstance(due_date_val, str):
                    try:
                        update_data["due_date"] = datetime.strptime(due_date_val, "%Y-%m-%d")
                    except Exception:
    ...
    def to_dict(task_doc):
        ...
        return {
            ...
            "due_date": to_iso(task_doc.get("due_date")),
            "created_at": to_iso(task_doc.get("created_at")),
            "updated_at": to_iso(task_doc.get("updated_at")),
        }